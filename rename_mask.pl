...
#             <lanemask (1s, 0s, single line, no spaces)>
#             
# Synopsis:
# Determine number of 1's <x> and columns <y> of lanemask, and
# rename it to "<x>-1s.<y>c.mask" if and only if that file
# doesn't already exist.
# Options from :
#    -L          : the lanemask file is a list of lanemask files
...
$usage = "Usage: perl rename_mask.pl\n\t<lanemask (1s, 0s, single line, no spaces) with <x> 1s of length <y>>\n\n\tFile will be copied to a new file named \"<x>-1s.<y>c.mask\n\n";
$options_usage  = "Options:\n\t";
$options_usage .= "-L     : the lanemask file is actually a list of lanemask files\n\t";
...
# make sure we only had 1s and 0s
    $lm =~ s/1//g;
    $lm =~ s/0//g;
    if($lm ne "") { printf("ERROR, lanemask has non-1/0 chars: $lm\n"); exit(1); }
